                        ...
                        <div className="flex items-start justify-between gap-4 flex-wrap">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                              ...
                              <span className={`capitalize text-xs font-bold px-2.5 py-0.5 rounded-full border ${cfg.bg} ${cfg.text} ${cfg.border}`}>
                                {r.status}
                              </span>
                            </div>

                            <h3 className="text-base font-bold text-gray-900 truncate">{r.parking?.name || '--'}</h3>
                          ...
                          <div className="flex-shrink-0 text-right">
                            <p className="text-2xl font-extrabold text-gray-900">{formatCurrency(r.total_amount)}</p>
                            <p className="text-xs text-gray-400 mt-0.5">total cost</p>
                            <p className="text-[10px] text-indigo-500 font-semibold mt-1 group-hover:underline">View Status →</p>
                            {r.entry_time && (
                              <p className="text-xs text-green-600 font-medium mt-1">Entered {formatDateTime(r.entry_time)}</p>
                            )}
                          </div>
                        </div>

                        {(r.status === 'completed' || ['pending','confirmed'].includes(r.status)) && (
                          <div className="mt-4 pt-3 border-t border-gray-50 flex items-center justify-between gap-2" onClick={e => e.stopPropagation()}>
                            {r.status === 'completed' ? (
                              <button onClick={() => downloadReceipt(r, r.parking)}
                                className="flex items-center gap-1.5 text-xs font-semibold text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 px-3 py-1.5 rounded-lg transition-all">
                            ...
                            {['pending','confirmed'].includes(r.status) && (
                              <button onClick={() => handleCancel(r)}
                                className="text-xs font-semibold text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 px-3 py-1.5 rounded-lg transition-all">
                                Cancel Reservation
                              </button>
                            )}
                          </div>
